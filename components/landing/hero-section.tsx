      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-center px-4 pb-20 pt-24 text-center md:pb-32 md:pt-32">
        ...
        <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-1.5">
          ...
          <span className="text-xs font-medium text-muted-foreground">
            {data?.badge || "Умная система сообщений о проблемах города"}
          </span>
        </div>
        ...
        <h1 className="max-w-4xl text-balance text-4xl font-bold leading-tight tracking-tight text-foreground md:text-6xl lg:text-7xl">
          {data?.titleFirst || "Сообщай. Приоритизируй."}{" "}
          <span className="text-primary">
            {data?.titleSecond || "Улучшай свой город."}
          </span>
        </h1>

        <p className="mx-auto mt-6 max-w-2xl text-pretty text-lg leading-relaxed text-muted-foreground md:text-xl">
          {data?.description || "CityFix автоматически объединяет похожие жалобы, показывает приоритеты на интерактивной карте и помогает быстрее решать городские проблемы."}
        </p>

        <div className="mt-10 flex flex-col gap-4 sm:flex-row">
          <Button asChild size="lg" className="gap-2 px-8">
            <Link href="/register">
              {data?.startButton || "Начать"}
          ...
          <Button asChild variant="outline" size="lg" className="px-8">
            <Link href="/#how-it-works">
              {data?.learnMoreButton || "Узнать больше"}
            </Link>
          </Button>
        </div>
